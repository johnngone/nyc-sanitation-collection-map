...
```bash
docker exec nyc-trash-map python scripts/run_refresh.py --status
docker exec nyc-trash-map python scripts/activate_release.py <dataset-version>
```
...
The scheduler runs inside the standalone container on startup when `DATA_REFRESH_ON_STARTUP=true`, then waits `DATA_REFRESH_INTERVAL_DAYS` between successful attempts. Every successful unchanged-source verification or validated release publication atomically advances the manifest pointer's `verified_at`; failed attempts do not advance it. The API exposes `verified_at` and preserves the release-build time as `processed_at`. A failure retains the current release and retries after `DATA_REFRESH_FAILURE_RETRY_MINUTES` (30 minutes by default; minimum one minute).
...
## Fresh deployment and rollback

Deploy with a new empty data directory when the existing data uses an incompatible persisted contract. Before switching, record the current image digest, dataset version, health/counts, per-type schedule counts, and disk usage; set the three first-release floors to at least 90% of the recorded LION, DSNY, and output-feature counts. Stop the old deployment, move its data directory to a timestamped backup, create an empty replacement, and start the new immutable image with startup refresh enabled.
...
If verification fails, stop the new deployment, preserve its failed data directory for diagnosis, restore the untouched backup, and redeploy the previous pinned image. Keep the backup through at least one later successful refresh and a tested activation rollback between retained v4 releases.
