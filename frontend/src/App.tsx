...
export function App() {
  ...
  const [dataVerified, setDataVerified] = useState<string | null>(null);
    ...
    const checkHealth = () => {
      ...
      void fetch(apiUrl("/api/health"), { signal: controller.signal })
        ...
        .then((payload: unknown) => {
          ...
          setDataVerified(payload.verified_at);
        })
        .catch((error: unknown) => {
  ...
  useEffect(() => {
    ...
    map.once("style.load", () => {
      ...
      const loadMapTiles = () => {
        ...
        .then((config) => {
          if (controller.signal.aborted) return;
          if (config.verified_at) setDataVerified(config.verified_at);
          if (!config.available) {
              ...
              <div className="advanced-controls-inner">
                ...
                <div className="status-summary-card">
                  ...
                  <dl className="status-summary" aria-live="polite">
                    ...
                    <div className="status-updated"><dt>Verified</dt><dd><time dateTime={dataVerified ?? undefined}>{formatVerifiedDate(dataVerified)}</time></dd></div>
                  </dl>
                </div>
              </div>
            </div>
...
function isValidHealthPayload(value: unknown): value is {
  ...
  return payload.status === "ok"
    ...
    && (payload.processed_at === null || typeof payload.processed_at === "string")
