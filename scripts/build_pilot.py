...
# Smaller traces no longer represent a credible street side.
MIN_RELIABLE_SIDE_OFFSET_RATIO = 0.25
    ...
    for prepared_number, prepared in enumerate(prepared_segments, start=1):
        ...
        for side, id_field, borough_field in SIDE_FIELDS:
            ...
            if invalid_matches:
                ...
                continue
            minimum_reliable_offset = side_offset_feet * MIN_RELIABLE_SIDE_OFFSET_RATIO
            if used_side_offset_feet < minimum_reliable_offset:
                result.status = "partially_outside_schedule_area"
                result.reason = (
                    "side-offset trace required an unreliable distance from the centerline"
                )
                result.details["minimum_reliable_side_offset_feet"] = minimum_reliable_offset
                result.unknown_geometry = geometry
                continue
...
def _matching_frequency_parts(
    ...
    matches = _collect_frequency_parts(
        frequencies,
        spatial_index,
        offset_pairs,
    )
    return matches, trace_length, used_side_offset_feet, offset_strategy
...
def _side_offset_pairs(
    ...
    requested_signed_distance = distance_feet if side == "LEFT" else -distance_feet
    ...
    primitive_pairs = _primitive_offset_pairs(source_lines, requested_signed_distance)
    if primitive_pairs:
...
def _collect_frequency_parts(
    ...
    for source_line, offset_line in offset_pairs:
        positions = spatial_index.query(offset_line, predicate="intersects")
        for position_value in positions:
            position = int(position_value)
            polygon = frequencies.geometry.iloc[position]
            try:
                intersection = offset_line.intersection(polygon)
            ...
            for trace_part in _line_parts(intersection):
                if trace_part.length < MIN_MAPPABLE_TRACE_FEET:
                    continue
                if not polygon.contains(trace_part.interpolate(0.5, normalized=True)):
                    continue
