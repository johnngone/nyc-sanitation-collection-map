...
def unchanged_release(
    ...
    *,
    ...
    try:
        ...
        return None
    # Promotion adds release history and verification time only to the atomic
    # pointer. Everything else in the installed manifest remains immutable.
    installed_binding = dict(installed)
    installed_binding.pop("previous_releases", None)
    installed_binding.pop("verified_at", None)
    current_binding = dict(current)
    current_binding.pop("previous_releases", None)
    current_binding.pop("verified_at", None)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix=".nyc-refresh-", dir=manifest_path.parent) as temporary:
        ...
        candidate_refresh_identity = refresh_fingerprint(
            processing_identity,
        ...
        if (
            ...
            reusable = unchanged_release(
                manifest_path,
                candidate_refresh_identity,
                regression_gate=regression_gate,
            )
            if reusable is not None:
                verified = record_release_verification(
                    manifest_path,
                    expected_version=str(reusable["dataset_version"]),
                    expected_fingerprint=candidate_refresh_identity,
                )
                LOGGER.info(
                    "Sources and processing fingerprint unchanged; retaining dataset version=%s "
                    "and skipping extraction plus stages 4-8 verified_at=%s",
