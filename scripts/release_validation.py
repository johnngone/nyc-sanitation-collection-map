...
def publish_release(
    ...
    *,
    ...
    with _promotion_lock(data_root):
        ...
        release_manifest["verified_at"] = datetime.now(UTC).isoformat()
...
def record_release_verification(
    ...
    *,
    ...
    """Atomically record a successful no-change verification."""

    pointer = Path(manifest_path).resolve()
    with _promotion_lock(pointer.parent):
        current = _read_current_manifest(pointer)
        if (
            current is None
            or current.get("dataset_version") != expected_version
            or current.get("refresh_fingerprint") != expected_fingerprint
        ):
            raise RuntimeError("current release changed during source verification")
        current["verified_at"] = datetime.now(UTC).isoformat()
