...
def test_default_branding_is_nyc_trash_map() -> None:
    assert config.APP_TITLE == "NYC Trash Map"
    assert config.APP_SUBTITLE == "See collection schedules by street and day."
    assert config.APP_BROWSER_TITLE == "NYC Trash Map"
    assert config.APP_SHORT_NAME == "NYC Trash Map"
    assert config.APP_META_DESCRIPTION == (
