...
def test_partially_covered_side_remains_entirely_unresolved() -> None:
    lion = lion_frame([
        lion_row(
            right_id=0,
            geometry=LineString([(X, Y), (X + 200, Y)]),
    ...
    unresolved = next(
        ...
        if feature["properties"]["reason_code"] == "PARTIAL_GEOMETRY_GAP"
    )
    assert shape(unresolved["geometry"]).length > 0
...
def test_subfoot_topology_gap_remains_unresolved() -> None:
    lion = lion_frame([lion_row(right_id=0)])
    frequencies = frequency_frame([
    ...
    assert payload["features"] == []
    assert any(
        feature["properties"]["reason_code"] == "OUTSIDE_DSNY_COVERAGE"
        for feature in payload["unknown_features"]
    )
    assert audit["outcomes"]["outside_schedule_area"] == 1


def test_polygon_boundary_contact_remains_unresolved() -> None:
    lion = lion_frame([lion_row(right_id=0)])
    frequencies = frequency_frame([{
        "OBJECTID": 802,
        "geometry": box(X - 10, Y + 25, X + 110, Y + 100),
    }])
...
def test_extremely_reduced_offset_remains_unresolved() -> None:
    curved = LineString([
        (X, Y),
        (X + 3, Y + 3),
        (X + 6, Y),
    ...
    lion = lion_frame([
        lion_row(
            segment_id="tiny-offset",
            left_id=0,
            right_id="uncertain-face",
            geometry=curved,
        ),
    ])
    frequencies = frequency_frame([{
    ...
    assert payload["features"] == []
    assert any(
        feature["properties"]["reason_code"] == "PARTIAL_GEOMETRY_GAP"
        for feature in payload["unknown_features"]
    )
    record = next(
        record
        for record in audit["records"]
        if record.get("block_face_id") == "uncertain-face"
    )
    assert record["used_side_offset_feet"] == pytest.approx(3.125)
    assert record["minimum_reliable_side_offset_feet"] == pytest.approx(6.25)
    assert record["outcome"] == "partially_outside_schedule_area"
...
def test_nonempty_but_truncated_offset_preserves_every_source_segment(
    ...
    payload, audit = build_collection_features(lion, frequencies, side_offset_feet=25)
    pairs, used_distance, strategy = _side_offset_pairs(geometry, "LEFT", 25)

    assert len(payload["features"]) == 1
    assert used_distance == 25
    assert strategy == "per_source_segment_offset"
    assert all(source.distance(offset) == pytest.approx(25) for source, offset in pairs)
    fallback_record = next(
        record
        for record in audit["records"]
