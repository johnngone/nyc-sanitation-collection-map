...
from backend.tests.release_fixtures import PROCESSED_AT, VERIFIED_AT, write_runtime_pointer
...
def test_map_config_and_tile_response(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(api, "DATA_MANIFEST_PATH", str(write_runtime_pointer(tmp_path / "data", archive)))
...
def test_map_config_prefers_manifest_verification_time(tmp_path, monkeypatch) -> None:
    archive = tmp_path / "verified-streets.mbtiles"
    _write_mbtiles(archive)
    pointer = write_runtime_pointer(
        tmp_path / "data",
        archive,
    ...
    config = TestClient(app).get("/api/map-config").json()

    assert config["processed_at"] == "2026-08-19T12:00:00Z"
    assert config["verified_at"] == "2026-09-15T20:34:00+00:00"


def test_runtime_rejects_pointer_without_verification_time(tmp_path, monkeypatch) -> None:
    archive = tmp_path / "unverified-streets.mbtiles"
    _write_mbtiles(archive)
    pointer = write_runtime_pointer(tmp_path / "data", archive)
