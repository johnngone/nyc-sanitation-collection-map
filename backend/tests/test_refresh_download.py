...
def _committed_refresh(tmp_path, processing: dict[str, object]):
    ...
    pointer_manifest["verified_at"] = "2026-09-15T20:34:00+00:00"
...
def test_unchanged_release_requires_exact_fingerprint_and_complete_artifacts(tmp_path) -> None:
    ...
    verified = record_release_verification(
        pointer,
        expected_version=manifest["dataset_version"],
        expected_fingerprint=candidate,
    )
    assert verified["processed_at"] == manifest["processed_at"]
    assert isinstance(verified["verified_at"], str)
    assert "verified_at" not in json.loads(
        (pointer.parent / manifest["release_path"] / "release_manifest.json").read_text(
            encoding="utf-8"
        )
    )
    assert unchanged_release(pointer, candidate, regression_gate=gate) == verified
...
def test_verification_rejects_a_concurrent_release_change(tmp_path) -> None:
    pointer, manifest, candidate = _committed_refresh(
        tmp_path,
        {"fingerprint_version": 1, "sha256": "b" * 64, "inputs": {}},
    )
    ...
    with pytest.raises(RuntimeError, match="changed during source verification"):
        record_release_verification(
            pointer,
            expected_version=f"{manifest['dataset_version']}-replaced",
            expected_fingerprint=candidate,
        )
